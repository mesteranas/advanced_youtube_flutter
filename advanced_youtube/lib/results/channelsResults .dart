...
class ChannelsSearchResults extends StatefulWidget{
  var q="";
  ChannelsSearchResults({Key?key,required this.q}):super(key:key);
  @override
...
class _ChannelsSearchResults extends State<ChannelsSearchResults>{
  ...
  var q="";
  _ChannelsSearchResults(this.q);
  Future<void> load() async{
    String url="";
    if (nextPageId.isEmpty){
      url='https://www.googleapis.com/youtube/v3/search?part=snippet&q=' + this.q + '&type=channel&maxResults=100&key=' + App.apiKey;
    }else{
      url='https://www.googleapis.com/youtube/v3/search?part=snippet&q=' + this.q + '&type=channel&maxResults=100&key=' + App.apiKey + '&pageToken=' + nextPageId;
    }
          ...
          onLongPress:(){
            showDialog(context: context, builder: (context){
              return AlertDialog(
                ...
                actions: [
                  IconButton(onPressed: (){
                    Clipboard.setData(ClipboardData(text: "https://www.youtube.com/channel/" + results[resultsKeys[index]]));
                    Navigator.pop(context);
                  }, icon: Icon(Icons.copy),tooltip: _("copy link"),),
                  IconButton(onPressed: (){
                    FlutterShare.share(title: "share",text: "https://www.youtube.com/channel/" + results[resultsKeys[index]]);
                    Navigator.pop(context);
